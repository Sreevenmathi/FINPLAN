...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $initial_balance = $_POST['acc_bal'];

    // Validate form inputs
    if (!empty($account_title) && !empty($account_type) && !empty($account_number) && !empty($initial_balance)) {
        // Sanitize inputs to prevent SQL injection
        $account_title = mysqli_real_escape_string($connection, $account_title);
        ...
        $initial_balance = mysqli_real_escape_string($connection, $initial_balance);

        // Prepare SQL query to insert data into the bank_accounts table
        $sql = "INSERT INTO bank_accounts (account_title, account_type, account_number, balance,user_id) 
                VALUES ('$account_title', '$account_type', '$account_number', '$initial_balance','$usr')";

        // Execute the query
        if ($connection->query($sql) === TRUE) {
            echo "<script>alert('Bank Account added successfully!');</script>";
           // echo "<script>location.replace('add_acc.php');</script>"; // Redirect to account list page
        } else {
            ...
            <div class="row g-4">
                ...
                <div class="col-sm-12 col-xl-6">
                    <div class="bg-secondary rounded h-100 p-4">
                        ...
                        <select class="form-select mb-3" name="acc_type" required>
                ...
                <div class="col-sm-12 col-xl-6">
                    <div class="bg-secondary rounded h-100 p-4">
                        ...
                        <div class="form-check form-switch">
                            <label class="form-check-label" for="flexSwitchCheckDefault">ACCOUNT TITLE: </label><br>
                            <label class="form-check-label" for="flexSwitchCheckDefault">ACCOUNT TYPE: </label><br>
                            <label class="form-check-label" for="flexSwitchCheckDefault">ACCOUNT NO:</label><br>
                            <label class="form-check-label" for="flexSwitchCheckDefault">ACCOUNT BALANCE:</label>
                        </div>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            ...
                            echo "<p><strong>ACCOUNT TYPE:</strong> " . ($acc_type == 1 ? 'Savings' : ($acc_type == 2 ? 'Current' : 'International')) . "</p>";
