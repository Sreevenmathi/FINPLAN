...
$sql = "SELECT goal_name, target_amount, start_date, target_date,id, monthly_contribution, priority_level, category, notes FROM goals WHERE user_id='$userid'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $progress = min((($row['monthly_contribution'] / $row['target_amount']) * 100), 100);
        ...
        echo "<td>" . round($progress) . "%</td>";
        echo "<td><a class='btn btn-sm btn-danger' href='delete_goal.php?id=" . urlencode($row['id']) . "'>DELETE</a></td>";
        echo "</tr>";
    }
} else {
